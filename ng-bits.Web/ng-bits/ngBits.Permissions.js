angular.module("ngBits.permissions", [])
	.service("permissions", ["$q", function ($q) {
		...
		this.hasPermission = function(permission, context) {
		...
		this.getContextPermissions = function(context, forceRefresh) {
			var entityType = context.entityType.shortName;
			var permissionsKey = entityType + "-" + context.entityAspect.getKey().toJSON().values.toString();
			...
			if (forceRefresh || angular.isUndefined(permissionArray)) {
				...
				if (handler) {
					handler(context).then(function(response) {
						var permissions = [];
